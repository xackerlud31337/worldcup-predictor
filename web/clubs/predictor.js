...
const NO_ADJUST = { ha: 1, hd: 1, aa: 1, ad: 1 };

// adj: player-availability multipliers from app.js — ha/aa scale each side's
// attack; hd/ad are defensive "leak" factors (>1 = missing defenders, so the
// OPPONENT's expected goals go up). Identity by default: with a full squad
// the player layer changes nothing.
function expectedGoals(bundle, home, away, neutral, adj = NO_ADJUST) {
  const [atkH, defH] = strength(bundle, home);
  const [atkA, defA] = strength(bundle, away);
  const ha = neutral ? 1.0 : bundle.home_adv;
  const lamHome = bundle.avg_goals * atkH * defA * ha * bundle.goal_scale * adj.ha * adj.ad;
  const lamAway = bundle.avg_goals * atkA * defH * bundle.goal_scale * adj.aa * adj.hd;
 ...
 * ---------------------------------------------------------------- */
function predictMatch(bundle, home, away, neutral = true, method = "dc", adj = NO_ADJUST) {
  const [lamHome, lamAway] = expectedGoals(bundle, home, away, neutral, adj);
...
function topScorelines(bundle, home, away, neutral = true, k = 6, adj = NO_ADJUST) {
  const [lamH, lamA] = expectedGoals(bundle, home, away, neutral, adj);
  const M = dcScoreMatrix(lamH, lamA, bundle.rho);
  const g = CFG.max_goals + 1;
  const all = [];
  for (let x = 0; x < g; x++)
...
function marginBreakdown(bundle, home, away, neutral = true, adj = NO_ADJUST) {
  const [lamH, lamA] = expectedGoals(bundle, home, away, neutral, adj);
  const M = dcScoreMatrix(lamH, lamA, bundle.rho);
  const g = CFG.max_goals + 1;
...
function bettingMarkets(bundle, home, away, neutral, pH, pD, pA, adj = NO_ADJUST) {
  const [lamH, lamA] = expectedGoals(bundle, home, away, neutral, adj);
  const M = dcScoreMatrix(lamH, lamA, bundle.rho);
  const g = CFG.max_goals + 1;
...
function runPrediction({ home_team, away_team, use_xg = false,
  ...
  const pred = predictMatch(bundle, home_team, away_team, neutral, method, adjust);
  ...
  return {
    ...
    margins: marginBreakdown(bundle, home_team, away_team, neutral, adjust),
    markets: bettingMarkets(bundle, home_team, away_team, neutral,
                            pred.p_home_win, pred.p_draw, pred.p_away_win, adjust),
    simulation: {
      top_scores: topScorelines(bundle, home_team, away_team, neutral, 6, adjust),
    },
  };
}
